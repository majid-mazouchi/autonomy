...
<div class="rl-minifig" data-concept="model-free">
  ...
  <canvas class="rl-minifig-canvas" height="170"></canvas>
...
(function() {
  ...
  var drawers = {
    ...
    /* MODEL-FREE: Agent (π_θ) ↔ black-box env, signal-level math on arrows */
    'model-free': function(ctx, W, H, t, colors) {
      var boxW = 95, boxH = 58;
      var agentX = 24, envX = W - 24 - boxW;
      var boxY = H / 2 - boxH / 2 - 4;

      // Agent box — shows π_θ inside
      box(ctx, agentX, boxY, boxW, boxH, colors.paper2, colors.rule, colors.ink, 'Agent', 'π_θ(a|s)', colors.ink);

      // Environment box — black-box styling
      ctx.fillStyle = '#3a342e';
      roundRect(ctx, envX, boxY, boxW, boxH, 4);
      ctx.fill();
      ctx.fillStyle = colors.paper;
      ctx.font = '500 12px "Fraunces", Georgia, serif';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText('Environment', envX + boxW / 2, boxY + boxH / 2 - 8);
      ctx.font = '9px "DM Mono", ui-monospace, monospace';
      ctx.globalAlpha = 0.6;
      ctx.fillText('unknown  P', envX + boxW / 2, boxY + boxH / 2 + 7);
      ctx.globalAlpha = 1;

      // Cycle — alternating pulse on top then bottom arrow
      var cycle = (t * 0.8) % 2;
      var topActive = cycle < 1, botActive = !topActive;
      var topPhase = topActive ? cycle : 0;
      var botPhase = botActive ? cycle - 1 : 0;

      var topY = boxY + 16;
      var botY = boxY + boxH - 16;

      // Top arrow: action a_t ~ π(·|s_t)
      arrow(ctx, agentX + boxW + 4, topY, envX - 4, topY,
            colors.accent, topActive, topPhase, topActive ? 2 : 1.25);
      ctx.fillStyle = topActive ? colors.accent : colors.inkMute;
      ctx.font = '500 10px "DM Mono", ui-monospace, monospace';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'bottom';
      ctx.fillText('aₜ ~ π(·|sₜ)', (agentX + boxW + envX) / 2, topY - 5);

      // Bottom arrow: r_t, s_{t+1}
      arrow(ctx, envX - 4, botY, agentX + boxW + 4, botY,
            colors.accent, botActive, botPhase, botActive ? 2 : 1.25);
      ctx.fillStyle = botActive ? colors.accent : colors.inkMute;
      ctx.textBaseline = 'top';
      ctx.fillText('rₜ,  sₜ₊₁', (agentX + boxW + envX) / 2, botY + 5);

      // Footer caption — the optimization target
      ctx.fillStyle = colors.inkMute;
      ctx.font = 'italic 10px "Newsreader", serif';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'bottom';
      ctx.fillText('optimize  𝔼[∑ γᵗ rₜ]  without ever modeling P', W / 2, H - 8);
    },

    /* MODEL-BASED: Agent/Env/f̂ triangle + diverging-trajectory miniplot */
    'model-based': function(ctx, W, H, t, colors) {
      var boxW = 78, boxH = 42;
      // Top row: Agent (left) + Env (right). Bottom row: Model f̂ (center).
      var topY = 10;
      var botY = H - boxH - 52;  // leave room for the drift miniplot
      var agentX = 14;
      var envX = W - 14 - boxW;
      var modelX = W / 2 - boxW / 2 - 30;  // model offset left; plot on right

      box(ctx, agentX, topY, boxW, boxH, colors.paper2, colors.rule, colors.ink, 'Agent', 'plan', colors.ink);
      box(ctx, envX, topY, boxW, boxH, colors.paper2, colors.rule, colors.ink, 'Env', 'real  f', colors.ink);

      // Model with faint glow when in use
      var glow = (Math.sin(t * 2) + 1) / 2;
      ctx.save();
      if (glow > 0.4) { ctx.shadowColor = colors.accent; ctx.shadowBlur = 5 * glow; }
      box(ctx, modelX, botY, boxW, boxH, colors.paper2, colors.accent, colors.accent, 'Model  f̂', 'learned', colors.ink);
      ctx.restore();

      // Real data flow: Env → Agent (top) and Env → Model (diagonal, dashed)
      arrow(ctx, envX, topY + boxH / 2, agentX + boxW + 4, topY + boxH / 2,
            colors.inkMute, false, 0, 1);
      arrow(ctx, envX + boxW / 2, topY + boxH + 2, modelX + boxW - 8, botY - 2,
            colors.inkMute, false, 0, 1);

      // Imagined rollouts: Model → Agent (pulsing accent)
      var cycle = (t * 1.2) % 1;
      arrow(ctx, modelX + 8, botY - 2, agentX + boxW / 2, topY + boxH + 2,
            colors.accent, true, cycle, 2);

      // Label for imagined rollouts
      ctx.fillStyle = colors.accent;
      ctx.font = '500 9px "DM Mono", ui-monospace, monospace';
      ctx.textAlign = 'right';
      ctx.textBaseline = 'middle';
      ctx.fillText('imagined  ŝₜ₊₁ = f̂(sₜ,aₜ)', modelX - 4, H / 2 - 8);

      // Diverging-trajectory miniplot on the right
      var plotX = envX - 8;
      var plotY = botY - 4;
      var plotW = 110;
      var plotH = 48;
      var plotLeft = W - plotW - 10;
      var plotTop = botY;
      ...
      ctx.textAlign = 'left';
      ctx.textBaseline = 'bottom';
      ctx.fillText('model error grows', plotLeft, plotTop - 2);

      // Real trajectory (solid, ink)
      ctx.strokeStyle = colors.ink;
      ctx.lineWidth = 1.4;
      ctx.beginPath();
      for (var i = 0; i <= 40; i++) {
        var xr = plotLeft + (i / 40) * plotW;
        var yr = plotTop + plotH * 0.55 + Math.sin(i * 0.35 + t * 0.5) * 4;
        if (i === 0) ctx.moveTo(xr, yr); else ctx.lineTo(xr, yr);
      }
      ctx.stroke();

      // Imagined trajectory (dashed, accent) — starts on top of real, drifts away
      ...
      for (var j = 0; j <= 40; j++) {
        var frac = j / 40;
        var drift = frac * frac * 22;  // quadratic drift
        var xi = plotLeft + frac * plotW;
        var yi = plotTop + plotH * 0.55 + Math.sin(j * 0.35 + t * 0.5) * 4 - drift;
        yi = Math.max(plotTop + 4, yi);
      ...
      ctx.fillText('— real', plotLeft + 4, plotTop + plotH - 16);
      ctx.fillStyle = colors.accent;
      ctx.fillText('- - imagined', plotLeft + 4, plotTop + plotH - 4);
      ...
      ctx.textAlign = 'center';
      ctx.textBaseline = 'bottom';
      ctx.fillText('keep imagined rollouts short — errors compound', W / 2, H - 6);
    },

    /* VALUE-BASED: Q(s, a) bars; argmax highlighted; explicit π(s) = argmax formula */
    'value-based': function(ctx, W, H, t, colors) {
      var n = 5;
      var labels = ['a₀', 'a₁', 'a₂', 'a₃', 'a₄'];
      var vals = [];
      var argmax = 0, bestV = -Infinity;
      for (var i = 0; i < n; i++) {
      ...
      var padL = 54, padR = 110, padT = 18, padB = 36;
      ...
      ctx.textAlign = 'right';
      ctx.textBaseline = 'middle';
      ctx.fillText('Q_θ(s, a)', padL - 6, padT + plotH / 2);
      ...
      var by = padT + plotH - plotH * vals[argmax];
      ctx.strokeStyle = colors.accent;
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.moveTo(bx, by - 4);
      ctx.lineTo(padL + plotW + 16, by - 4);
      ctx.stroke();
      ...
      ctx.textBaseline = 'middle';
      ctx.font = '500 10px "DM Mono", ui-monospace, monospace';
      ctx.fillText('π(s) = arg maxₐ Q', padL + plotW + 18, by - 4);

      // Footer: TD update formula
      ctx.fillStyle = colors.inkMute;
      ctx.font = 'italic 10px "Newsreader", serif';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'bottom';
      ctx.fillText('update:  Q(sₜ,aₜ) ← Q + α[rₜ + γ maxₐ Q(sₜ₊₁,a) − Q]', W / 2, H - 8);
    },

    /* POLICY-BASED: Gaussian π_θ with mean drifting; explicit gradient formula */
    'policy-based': function(ctx, W, H, t, colors) {
      var padL = 30, padR = 20, padT = 18, padB = 38;
      var plotW = W - padL - padR;
      var plotH = H - padT - padB;
      ...
      for (var a = -2; a <= 2; a++) {
        ...
        ctx.fillText(a.toString(), ax, padT + plotH + 4);
      }
      ctx.fillText('action  a', padL + plotW / 2, padT + plotH + 16);
      ...
      for (var i = 0; i <= N; i++) {
        ...
        var y = padT + plotH - (pdf / maxPdf) * plotH * 0.78;
      ...
      for (var i2 = 0; i2 <= N; i2++) {
        ...
        var y2 = padT + plotH - (pdf2 / maxPdf) * plotH * 0.78;
      ...
      var muY = padT + plotH - plotH * 0.78;
      ctx.fillStyle = colors.ink;
      ctx.beginPath();
      ctx.arc(muX, muY, 3.5, 0, 2 * Math.PI);
      ctx.fill();
      ...
      ctx.textAlign = 'left';
      ctx.textBaseline = 'middle';
      ctx.fillText('π_θ(a|s)', padL + 4, padT + 10);

      // Gradient formula as footer
      ctx.fillStyle = colors.inkMute;
      ctx.font = 'italic 10px "Newsreader", serif';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'bottom';
      ctx.fillText('∇_θ J  ≈  𝔼[∇_θ log π_θ(a|s) · A(s,a)]', W / 2, H - 6);
    },

    /* ACTOR-CRITIC: Actor + Critic, with δ computed inside critic and two losses shown */
    'actor-critic': function(ctx, W, H, t, colors) {
      var boxW = 104, boxH = 52;
      var actorX = 18;
      ...
      // Critic — pulse when judging
      var judge = (Math.sin(t * 1.6) + 1) / 2;
      var critFill = judge > 0.5 ? colors.accent : colors.paper2;
      var critBorder = critFill === colors.accent ? colors.accent : colors.rule;
      var critText = critFill === colors.accent ? colors.paper : colors.ink;
      box(ctx, criticX, y, boxW, boxH, critFill, critBorder, critText, 'Critic', 'V_φ(s)', critText);

      // Compute δ formula shown floating just below critic when active
      if (judge > 0.5) {
        ctx.fillStyle = colors.paper;
        ctx.font = '8px "DM Mono", ui-monospace, monospace';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'top';
        ctx.fillText('δ = r + γV(s\u2032) − V(s)', criticX + boxW / 2, y + boxH - 10);
      }

      // Top arrow: Actor → Critic (action)
      ...
      ctx.textAlign = 'center';
      ctx.textBaseline = 'bottom';
      ctx.fillText('aₜ ~ π_θ(·|sₜ)', (actorX + boxW + criticX) / 2, y + 10);

      // Bottom arrow: Critic → Actor (TD error / advantage)
      ...
      ctx.textAlign = 'center';
      ctx.textBaseline = 'top';
      ctx.fillText('advantage  Â_t ≈ δ_t', (actorX + boxW + criticX) / 2, y + boxH - 10);

      // Two loss labels — one under each box
      ctx.font = 'italic 9px "Newsreader", serif';
      ctx.fillStyle = colors.inkSoft;
      ctx.textAlign = 'center';
      ctx.textBaseline = 'top';
      ctx.fillText('actor loss:  − log π · Â', actorX + boxW / 2, y + boxH + 8);
      ctx.fillText('critic loss:  (V − target)²', criticX + boxW / 2, y + boxH + 8);
      ...
      ctx.textAlign = 'center';
      ctx.textBaseline = 'bottom';
      ctx.fillText('(r, s\u2032 from env — shared between both)', W / 2, H - 6);
    },

    /* ON-POLICY: Buffer fills → update flash → empties, with μ = π label + cycle */
    'on-policy': function(ctx, W, H, t, colors) {
      var bufferX = W / 2 - 90;
      var bufferY = 34;
      var bufferW = 180, bufferH = 42;
      var nSlots = 8;
      var slotW = (bufferW - 10) / nSlots;

      // Cycle: fill (0–1) → flash update (1–1.4) → clear (≥1.4)
      var cycle = (t * 0.6) % 2.2;
      ...
      ctx.textAlign = 'center';
      ctx.textBaseline = 'bottom';
      ctx.fillText('data ~ π_θ   (behavior policy μ = π)', bufferX + bufferW / 2, bufferY - 14);

      ctx.fillStyle = colors.inkMute;
      ctx.font = '9px "DM Mono", ui-monospace, monospace';
      ctx.fillText('rollout buffer', bufferX + bufferW / 2, bufferY - 3);
      ...
      ctx.fillStyle = (cycle > 1 && cycle < 1.4) ? colors.accent : colors.inkMute;
      ctx.font = 'italic 10px "Newsreader", serif';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'top';
      ctx.fillText(phaseLabel, W / 2, bufferY + bufferH + 8);

      // Cycle annotation at the bottom
      ctx.fillStyle = colors.inkSoft;
      ctx.font = '500 9px "DM Mono", ui-monospace, monospace';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'bottom';
      ctx.fillText('collect  →  update  →  repeat', W / 2, H - 6);
    },

    /* OFF-POLICY: Persistent buffer, μ ≠ π label, IS ratio on sampling arrow */
    'off-policy': function(ctx, W, H, t, colors) {
      var bufferX = W / 2 - 110;
      var bufferY = 34;
      var bufferW = 220, bufferH = 42;
      var nSlots = 14;
      var slotW = (bufferW - 10) / nSlots;
      ...
      ctx.textAlign = 'center';
      ctx.textBaseline = 'bottom';
      ctx.fillText('data ~ μ  (past policies)    π = target', bufferX + bufferW / 2, bufferY - 14);

      ctx.fillStyle = colors.inkMute;
      ctx.font = '9px "DM Mono", ui-monospace, monospace';
      ctx.fillText('replay buffer (persistent)', bufferX + bufferW / 2, bufferY - 3);
      ...
      // Buffer fills over t then stays full
      var fillRatio = Math.min(1, t / 5);
      var filled = Math.floor(fillRatio * nSlots);

      // Sampling — random slots highlight each ~second
      ...
      var phaseLabel = samplingActive
        ? '→ sample mini-batch   ρ = π(a|s) / μ(a|s)'
      ...
      ctx.textAlign = 'center';
      ctx.textBaseline = 'bottom';
      ctx.fillText('append  +  sample  (any past transition)  →  update', W / 2, H - 6);
    }
  };
  ...
  function resizeAll() {
    figures.forEach(function(fig) {
      ...
      fig.cv.height = 170 * dpr;
      fig.cv.style.height = '170px';
